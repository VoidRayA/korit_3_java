...
## 필요 이유
캡슐화(encapsulation) - 내부에 복잡한 동작을 외부에 노출
...
System.out.println(car1.model1);을 했을 때 셀토스가 콘솔에 찍히고
...
public class CarMain {
    public static void main(String[] args) {
        // 객체 생성
        Car car1 = new Car();
        System.out.println(car1.engine);    // 컴파일링 오류
...
```Java
    ...
    System.out.println("체력 : " + character1.getHealth());
```
이상과 같은 차이점이 나타난 이유에 대해서는 명확히 알 필요가 있습니다.
...
자료형명[] 변수명 = 
ex)
int[] arr1 = enw int[5];    // 배열 선언 방식 # 1 -> 배열 선언만 하고
값은 나중에 대입
...
일반 변수의 참조 변수라는 개념에 대해서 학습할 필요가 있습니다.
